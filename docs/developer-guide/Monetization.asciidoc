...
. `synchronizeReceipts(milliSeconds)` - Synchronizes the receipts with the receipt store. This will try to submit any pending purchase receipts to the receipt store, and the reload receipts from the receipt store.
...
Notice that, in `itemPurchased()` you don't need to explicitly create any receipts or submit anything to the receipt store. This is handled for you automatically. You do make a call to `synchronizeReceiptsSync(long ifOlderthanMs)` but this is just to ensure that your toast message has the new expiry date loaded already.
